...
public abstract class BaseBottomDelegate extends AbstractViewPlusDelegate implements
   ...
   * 存储tabs视图
   */
  protected final ArrayList<AbstractViewPlusDelegate> ITEM_DELEGATES = new ArrayList<>();
   ...
   * 选中的tab高亮的颜色
   */
  protected int mClickedColor = Color.RED;
   ...
   * 图标大小
   */
  protected int mTabIconSize = 19;

  /**
   * 底部tab 文字大小
   */
  protected int mTabTitleSize = 10;
   ...
   * @return
   */
  public abstract LinkedHashMap<BottomTabBean, AbstractViewPlusDelegate> setItems(
      ItemBuilder builder);
  ...
  public abstract int setTabIconSize();

  public abstract int setTabTitleSize();
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if (setTabIconSize() > 19) {
      mTabIconSize = setTabIconSize();
    }
    if (setTabTitleSize() > 10) {
      mTabTitleSize = setTabTitleSize();
    }
  ...
  public void onBindView(@Nullable Bundle savedInstanceState, @NonNull View rootView) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      itemIcon.setTextSize(mTabIconSize);
  ...
  protected void resetColor() throws ViewPlusException {
    try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        grayColor = getResources()
            .getColor(R.color.icon_no_selector, _mActivity.getTheme());
        // grayColor = ContextCompat.getColor(getContext(), R.color.icon_no_selector);
      } else {
        //noinspection AliDeprecation
        grayColor = getResources().getColor(cn.jiiiiiin.vplus.core.R.color.icon_no_selector);
      }
